...
class Model(nn.Module):
    ...
    def __init__(self):
        ...
        self.model_trans = BT_Transformer(length = 168,
                                          d_model = 448,
                                          dropout = 0.0,
                                          num_layers = 12,
                                          nhead = 16,
                                          ...
                                          att_dropout = 0.05,
                                          proj_dropout = 0.05)
        self.model_trans_projector = nn.Linear(448, 256)
        ...
        self.model_c3d_projector = nn.Linear(2048, 64)

        self.last_hitter_projector = nn.Sequential(
            nn.Linear(2, 128),
            nn.ReLU(),
            nn.Linear(128, 128))
    ...
    def forward(self, videos, last_hitter, ball, A, B, net, court):
        bs, N, C, T, H, W = videos.size()
        videos = videos.view(bs*N, C, T, H, W)
        outs = self.model_c3d(videos)
        video_hs = outs.view(bs, N, -1)
        # print(video_hs.size()) # [4, 5, 2048]
        video_hs = self.model_c3d_projector(video_hs)
        video_hs = video_hs.flatten(1)

        seq_hs = self.model_trans(ball, A, B, net, court)
        # print(seq_hs.size()) # [4, 448]
        seq_hs = self.model_trans_projector(seq_hs)

        hitter_hs = self.last_hitter_projector(last_hitter)
        # print(hitter_hs.size()) # [4, 128]
        x = torch.cat((video_hs, seq_hs, hitter_hs), dim=1)
        x = self.classifier(x)
        return x
